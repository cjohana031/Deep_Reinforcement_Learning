...
from environments.shadow_hand_reach_env import ShadowHandReachEnvironment, SuccessInfoWrapper
...
def train_sac_parallel(episodes=5000,
    ...
    Args:
        ...
        max_episode_steps: Maximum steps per episode (default 50 for HandReach)
        vec_env_cls: Vectorized environment class (SubprocVecEnv or DummyVecEnv)
    """
    print("=" * 80)
    print("STARTING PARALLEL SAC TRAINING ON SHADOW DEXTEROUS HAND REACH")
    print("=" * 80)
    ...
    def env_factory():
        base_env = ShadowHandReachEnvironment(
    ...
    wrapped_env = env_factory()
    single_env = wrapped_env.env  # Get the underlying ShadowHandReachEnvironment
    ...
    # SAC hyperparameters optimized for Shadow Hand Reach
    sac_params = {
        'learning_rate': 3e-4,  # Standard for robotics tasks
        'buffer_size': 1_000_000,
        'learning_starts': 10000,  # Start learning after collecting data
        'batch_size': 256,
        ...
        'use_sde': False,
        'policy_kwargs': dict(
            net_arch=dict(pi=[512, 512, 256], qf=[512, 512, 256]),  # Larger networks for complex task
...
def evaluate_parallel_agent(model_path='models/sac_shadow_hand/sac_best.zip', episodes=10):
    """Evaluate a trained parallel agent"""
    print("🔍 Evaluating trained Shadow Hand Reach agent...")
    ...
    try:
        ...
        for i in range(episodes):
            ...
            while True:
                ...
                # Extract goal distance if available
                if hasattr(env, 'env') and hasattr(env.env, 'env'):
                    # Get the actual gym environment
                    gym_env = env.env.env
                    if hasattr(gym_env, 'unwrapped'):
                        unwrapped = gym_env.unwrapped
                        # Try to get distance info
                        if hasattr(unwrapped, 'goal_distance'):
                            final_distance = unwrapped.goal_distance()
            ...
            print(f"Episode {i+1:2d}: Reward = {episode_reward:7.2f}, Success = {success:.2f}, Distance = {final_distance:.4f}")
        ...
        print(f"Success Episodes: {np.sum(np.array(total_successes) > 0.5)} / {episodes}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train SAC agent with parallel environments on Shadow Hand Reach')
    ...
    parser.add_argument('--reward_scale', type=float, default=1.0,
                        help='Scaling factor for rewards (default 1.0 = no scaling)')
