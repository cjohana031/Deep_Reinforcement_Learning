...
class SB3SACAgent:
    """Wrapper around Stable Baselines3 SAC implementation for Shadow Hand Reach"""
    ...
    def __init__(
        ...
        # Set default policy kwargs optimized for Shadow Hand (higher complexity)
...
class TrainingCallback(BaseCallback):
    """
    Custom callback for SAC training on Shadow Hand Reach to track metrics and save checkpoints
    """
    def __init__(self,
                 eval_freq: int = 1000,
                 save_freq: int = 10000,
    ...
    def _on_rollout_end(self) -> None:
        ...
        if len(self.model.ep_info_buffer) > 0:
            ...
            success = 0.0
            # Check if episode info contains success information from our wrapper
            if isinstance(ep_info, dict) and 'episode' in ep_info and 'is_success' in ep_info['episode']:
                success = float(ep_info['episode']['is_success'])
            ...
            if self.custom_logger:
                ...
                self.custom_logger.writer.add_scalar('Episode/Success_Rate', success, self.episode_count)
                ...
                if len(self.episode_rewards) >= 10:
                    ...
                    success_10 = np.mean(self.episode_successes[-10:])
                    self.custom_logger.writer.add_scalar('Average/Reward_10ep', avg_10, self.episode_count)
                    self.custom_logger.writer.add_scalar('Average/Success_10ep', success_10, self.episode_count)

                if len(self.episode_rewards) >= 100:
                    avg_100 = np.mean(self.episode_rewards[-100:])
                    success_100 = np.mean(self.episode_successes[-100:])
                    self.custom_logger.writer.add_scalar('Average/Reward_100ep', avg_100, self.episode_count)
                    self.custom_logger.writer.add_scalar('Average/Success_100ep', success_100, self.episode_count)
            ...
            if self.episode_count % 10 == 0:
                ...
                print(f"[TRAINING] Episode {self.episode_count:6d} | "
                      f"Reward: {ep_info['r']:8.2f} | "
                      f"Avg100: {mean_reward:8.2f} | "
                      f"Success: {success:.2f} | "
                      f"Avg100Success: {success_rate:.3f} | "
                      f"Steps: {self.num_timesteps:8d}")

                # Save best model based on success rate and reward
                if success_rate > self.best_success_rate or (success_rate == self.best_success_rate and mean_reward > self.best_mean_reward):
                    self.best_mean_reward = mean_reward
                    self.best_success_rate = success_rate
                    if self.save_path:
                        self.model.save(f"{self.save_path}/sac_best")

            # Save checkpoint (save_freq is in timesteps, convert to episodes)
            episodes_per_save = self.save_freq // 50  # Assuming 50 steps per episode
            if self.save_freq > 0 and episodes_per_save > 0 and self.episode_count % episodes_per_save == 0:
                if self.save_path:
