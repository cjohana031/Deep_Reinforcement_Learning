#!/usr/bin/env python3
"""
Evaluation script for SB3 TD3 agent on FetchReachDense-v4 environment.

This script evaluates a trained SB3 TD3 agent and can optionally render the environment
...
from agents.td3_sb3 import SB3TD3Agent
from environments.hand_reach_env import make_hand_reach_env


class SB3TD3Evaluator:
    """Evaluates a trained SB3 TD3 agent."""
    ...
    def setup_agent(self):
        """Setup and load the trained SB3 TD3 agent."""
        print("Setting up SB3 TD3 agent...")

        # Create a dummy environment factory for SB3 agent initialization
        ...
        if self.args.model_path:
            ...
            model_path = str(Path(self.args.model_dir) / "td3_best")
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        """
        print(f"\nEvaluating SB3 TD3 agent for {self.args.num_episodes} episodes...")
        print("=" * 60)
    ...
    def save_results(self, stats: Dict[str, float]):
        ...
        model_path_str = self.args.model_path if self.args.model_path else str(Path(self.args.model_dir) / "td3_best")
        ...
        results_path = Path(self.args.results_dir) / f"evaluation_results_sb3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Evaluate SB3 TD3 agent on FetchReachDense-v4')

    # Model parameters
    parser.add_argument('--model-path', type=str, default=None,
                        help='Path to trained model (default: model_dir/td3_best)')
    ...
    print("SB3 TD3 Evaluation for FetchReachDense-v4")
    print("=" * 40)
    ...
    evaluator = SB3TD3Evaluator(args)
    evaluator.run()
