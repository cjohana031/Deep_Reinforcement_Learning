...
AVERAGE_STEPS_PER_EPISODE = 1000  # Approximate average steps per episode for DoubleDunk

def train_ddqn(episodes=50000, eval_interval=10, save_interval=10, render=False):
    print("=" * 70)
    print("STARTING DDQN TRAINING ON DOUBLEDUNK")
    print("=" * 70)
    ...
    # Initialize agent and updater with optimized hyperparameters for Double Dunk
    agent = DDQNAgent(action_dim=action_dim)
    updater = DDQNUpdater(
        agent, 
        device=agent.device, 
        epsilon_decay_steps=500000,  # Faster exploration decay
        warmup_steps=50000,          # Warm-up period
        target_update_freq=8000,     # More frequent target updates
    ...
    for episode in range(1, episodes + 1):
        ...
        # Log episode with metrics
        avg_loss = np.mean(episode_loss) if episode_loss else None
        logger.log_episode(episode, episode_reward, episode_steps, td_loss=avg_loss, epsilon=updater.epsilon, network_loss=avg_loss)
        
        # Log training metrics
        if episode_loss:
            logger.log_training_metrics(episode, loss=avg_loss, epsilon=updater.epsilon, td_loss=avg_loss)
        
        # Print progress - more frequent for first 1000 episodes, then every 100
        log_interval = 1 if episode <= 1000 else 50
